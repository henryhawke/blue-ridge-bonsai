...
async function setupMemberDashboard() {
  ...
  // Use the async widget initializer below
...
function displayMemberInfo() {
  $w(
    "#welcomeMessage"
  ).html = `<h1>Welcome back, ${currentMember.firstName}!</h1>`;
  ...
  $w("#memberInfo").html = `
        <div class="glass-card">
            ...
            <p><strong>Name:</strong> ${currentMember.firstName} ${
...
async function saveBonsaiToPortfolio() {
  try {
    ...
    await loadMemberPortfolio(currentMember);
...
async function loadUpcomingEvents() {
  ...
  const upcomingEvents = registrations.filter(
    (reg) => new Date(reg.startDate) > new Date()
  );

  let eventsHTML = "<h3>Your Upcoming Registered Events</h3>";
  if (upcomingEvents.length > 0) {
    eventsHTML += upcomingEvents
      .map(
        (event) => `
...
async function initializeDashboardWidgets() {
  ...
  $w("#membershipStatus").html = `
        <div class="glass-card">
            ...
            ${
              status.isActive
                ? `<p>Expires in ${status.daysUntilExpiry} days.</p>`
                : ""
            }
            ${
              status.needsRenewal
  ...
  $w("#memberStats").html = `
        <div class="glass-card">
            <h3>Your Stats</h3>
            <p><strong>Trees in Portfolio:</strong> ${
              currentMember.bonsaiCollection
                ? currentMember.bonsaiCollection.split(",").length
                : 0
            }</p>
            <p><strong>Years of Experience:</strong> ${
              currentMember.yearsExperience || "New"
            }</p>
  ...
  if (interactions.length > 0) {
    activityHTML += interactions
      .slice(0, 3)
      .map((item) => `<p>${item.type}: ${item.description || ""}</p>`)
      .join("");
  } else {
